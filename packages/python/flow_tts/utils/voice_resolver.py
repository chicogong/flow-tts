...
class VoiceResolver:
    ...
    def _init(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load voice library: {e}. " f"Expected location: {data_dir}"
            )
    ...
    def search_voices(self, query: str) -> List[Voice]:
        ...
        for voice in all_voices:
            if (
                query_lower in (voice.get("name") or "").lower()
                or query_lower in (voice.get("description") or "").lower()
                or query_lower in (voice.get("language") or "").lower()
            ):
